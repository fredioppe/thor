    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">              
        ...
        //echo $this->Form->Button('Salvar',['name'=>'salvarmodal','id' =>'btnsalvar', 'data-dismiss'=>'modal', 'controller'=>'users', 'action'=>'add']);
          ?>
        <button type="button" id="btnCancelar" onclick=location.href='<?php 
        use Cake\Routing\Router;
        echo Router::url(array('controller'=>'Users', 'action'=>'index')); 
        ?>'  data-dismiss="modal" class="btn btn-primary"><?= __('Cancelar') ?></button>
      </div>                                        
    </div>
  </div>
</div>